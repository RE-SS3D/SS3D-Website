...
The updated animator's kit (v1.5) can be found [here]({{ site.animators_kit_url }}) (download the whole folder).
...
Finally, **CosmicCoincidence** added an file full of [example models]({{ site.baseurl }}/assets/img/posts/21.08.01/ExampleModels.png) (.blend) which can be used as a quick reference for scale, styling, etc.

The updated modeler's kit (v2.0) can be found [here]({{ site.modelers_kit_url }}) (download the whole folder).
...
The upgraded system is still tile-based but leverages collections more to process through objects on the same layer a bit faster. It also now divides the tilemap into [16 x 16 chunks]({{ site.baseurl }}/assets/img/posts/21.08.01/TilemapChunks.png) which should allow for a more optimized system as well.
...
**Ryan**
- Added a [new hover-based examination feature]({{ site.baseurl }}/assets/img/posts/21.08.01/HoverExamine.png) which only displays object names as opposed to the detailed examine which can have a description, a UI, or other additions.
