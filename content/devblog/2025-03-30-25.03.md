...
Those willing to assist in broadening the language support for SS3D can join our {{ link(key="discord_url", text="Discord", path="") }} and assist a programmer in getting their language implemented.
...
**Nath** added to the construction system recently, we now have building recipes for walls.
